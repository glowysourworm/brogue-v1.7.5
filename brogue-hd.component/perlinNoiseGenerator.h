...
namespace brogueHd::component
{
								   ...
								   gridCallbackConst<bool> callback)
	{
		...
		//
		int meshWidth = (int)(relativeBoundary.width / (double)_meshSizeX) + 2;
		// Let it overhang (see coordinate transforms)
	...
	double perlinNoiseGenerator::perlinFade(double unitValue)
	{
		if (unitValue > 1.0 ||
			...
			throw simpleException(
				"Trying to generate Perlin easing value out of bounds:  perlinNoiseGenerator::perlinFade");

		// https://rmarcus.info/blog/2018/03/04/perlin-noise.html (this may be worth parameterizing and playing with; but it produces
		//                                                         good results)
		//
		return (6 * simpleMath::power(unitValue, 5)) - (15 * simpleMath::power(unitValue, 4)) + (10 *
			simpleMath::power(unitValue, 3));
	}
}
