...
namespace brogueHd::component
{
	...
	public:
		...
		/// </summary>
		int randomRangeInclusive(int lowerBound, int upperBound, randomVariable type = randomVariable::Uniform,
		                         float mean = 0.5f, float stdDev = GAUSS_STD_DEV);

		/// <summary>
		/// Gets a random integer between lower and upper bounds, exclusive upper bound, with
		/// specified probability distribution.
		/// </summary>
		int randomRangeExclusive(int lowerBound, int upperBound, randomVariable type = randomVariable::Uniform,
		                         float mean = 0.5f, float stdDev = GAUSS_STD_DEV);
		...
		/// </summary>
		template <isNumber T>
		int randomIndex(const simpleArray<T>& weights, randomVariable type = randomVariable::Uniform, float mean = 0.5f,
		                float stdDev = GAUSS_STD_DEV);
		...
		/// </summary>
		color nextColor(color low, color high, randomVariable type = randomVariable::Uniform, float mean = 0.5f,
		                float stdDev = GAUSS_STD_DEV);

	private:
		int nextInt(int lowerBound, int upperBound, bool exclusiveUpperBound, randomVariable type, float mean,
		            float stdDev);
	...
	int randomGenerator::nextInt(int lowerBound, int upperBound, bool exclusiveUpperBound, randomVariable type,
	...
	int randomGenerator::randomRangeInclusive(int lowerBound, int upperBound, randomVariable type, float mean,
	                                          float stdDev)
	{
		return nextInt(lowerBound, upperBound, false, type, mean, stdDev);
	}

	int randomGenerator::randomRangeExclusive(int lowerBound, int upperBound, randomVariable type, float mean,
	                                          float stdDev)
	{
