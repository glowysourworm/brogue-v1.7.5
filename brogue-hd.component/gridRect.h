...
namespace brogueHd::component
{
	...
	public:
		...
		/// </summary>
		void iterateOutsideAdjacent(int distance, const gridRect& constraint,
		                            gridRectDirectionalIterator callback) const
		{
			...
			{
				// Top
				for (int column = left() - count; column <= right() + count && constraint.contains(
					     column, top() - count); column++)
				{
				...
				// Bottom
				for (int column = right() + count; column >= left() - count && constraint.contains(
					     column, bottom() + count); column--)
				{
					// Repeat
