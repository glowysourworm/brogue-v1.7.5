...
namespace brogueHd::backend::model::layout
{
	...
	gridRect brogueGrid::getBoundary() const
	{
		return _grid->getBoundary();
	}

	bool brogueGrid::isDefined(short column, short row) const
	{
		return !_grid->isZeroValue(column, row);
	}

	brogueCell* brogueGrid::getCell(short column, short row) const
	{
		return _grid->get(column, row);
	}

	void brogueGrid::addCell(short column, short row)
	{
	...
	void brogueGrid::addCells(gridRegion<short>* region, function<bool(short)> predicate)
	{
	...
	void brogueGrid::addCells(gridRect boundary)
	{
		brogueGrid* that = this;
	...
	void brogueGrid::addCellsInCircle(gridRect boundary)
	{
	...
	void brogueGrid::removeCell(short column, short row)
	{
	...
	void brogueGrid::removeCells(gridRect boundary)
	{
		brogueGrid* that = this;
	...
	void brogueGrid::removeCellsInCircle(gridRect boundary)
	{
		brogueAssert(boundary.width == boundary.height);
	...
	void brogueGrid::iterateAdjacentCells(short column, short row, function<bool(short, short, brogueCell*)> callback)
	{
	...
	brogueCell* brogueGrid::checkAdjacentCells(short& column, short& row, function<bool(brogueCell*)> callback)
	{
		grid<brogueCell*>* grid = _grid;
		brogueCell* result;
