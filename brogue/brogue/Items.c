...
item *initializeItem() {
	...
	theItem->inventoryColor = &whiteBrogue;
...
char displayInventory(unsigned short categoryMask,
	...
	encodeMessageColor(whiteColorEscapeSequence, 0, &whiteBrogue);
	grayColorEscapeSequence[0] = '\0';
	encodeMessageColor(grayColorEscapeSequence, 0, &grayBrogue);
	...
	for (i=0; i<itemNumber; i++) {
		...
		itemName(theItem, buf, true, true, (buttons[i].flags & B_HOVER_ENABLED) ? &whiteBrogue : &grayBrogue);
		upperCase(buf);
...
boolean chooseTarget(short returnLoc[2],
	...
	do {
		...
		if (!targetConfirmed) {
			...
			hiliteCell(targetLoc[0], targetLoc[1], &whiteBrogue, (cursorInTrajectory ? 100 : 35), true);
		}
...
boolean useStaffOrWand(item *theItem, boolean *commandsRecorded) {
    ...
    if (!boltKnown) {
        trajectoryHiliteColor = grayBrogue;
...
void readScroll(item *theItem) {
	...
	switch (theItem->kind) {
		...
		case SCROLL_AGGRAVATE_MONSTER:
			aggravateMonsters(DCOLS + DROWS, player.xLoc, player.yLoc, &grayBrogue);
...
item *itemAtLoc(short x, short y) {
	...
	if (theItem == NULL) {
		pmap[x][y].flags &= ~HAS_ITEM;
		hiliteCell(x, y, &whiteBrogue, 75, true);
