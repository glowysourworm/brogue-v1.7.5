...
#include "command.h"
#include "blueprint.h"
#include "blueprintDeclaration.h"
#include "feat.h"
#include "dungeon.h"
...
void initializeMenuButtons(buttonState *state, brogueButton buttons[5]) {
    ...
    encodeMessageColor(goldTextEscape, 0, KEYBOARD_LABELS ? &yellow : &whiteBrogue);
	encodeMessageColor(whiteTextEscape, 0, &whiteBrogue);
...
void getCellAppearance(short x, short y, uchar *returnChar, color *returnForeColor, color *returnBackColor) {
	...
	if (!playerCanSeeOrSense(x, y)
		...
		if (pmap[x][y].flags & HAS_PLAYER) {
			...
			cellForeColor = whiteBrogue;
			lightMultiplierColor = whiteBrogue;
...
//	 DEBUG if (cellHasTerrainFlag(x, y, T_IS_FLAMMABLE)) cellBackColor.red += 50;
	...
	if (rogue.trueColorMode
		...
		if (displayDetail[x][y] == DV_DARK) {
			...
			applyColorAugment(&cellBackColor, &whiteBrogue, -10);
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			if (pmap[i][j].layers[DUNGEON] != GRANITE
				...
				plotCharWithColor(' ', mapToWindowX(i), mapToWindowY(j), &whiteBrogue, &black);
			}
	...
	for (n=(invert ? stepCount - 1 : 0); (invert ? n >= 0 : n <= stepCount); n += (invert ? -1 : 1)) {
		for (i=0; i<COLS; i++) {
			for (j=0; j<ROWS; j++) {
				...
				foreColor = (invert ? whiteBrogue : black);
    ...
    for (i=0; i<DCOLS; i++) {
        for (j=0; j<DROWS; j++) {
            ...
            if (lowestDistance < 10) {
                hiliteCell(i, j, &whiteBrogue, clamp(100 - lowestDistance*15, 0, 100), true);
            }
        }
    }
...
boolean getInputTextString(char *inputText,
	...
	if (useDialogBox) {
		...
		overlayDisplayBuffer(dbuf, rbuf);
		printString(prompt, x, y - 1, &whiteBrogue, &interfaceBoxColor, NULL);
		for (i=0; i<maxLength; i++) {
			plotCharWithColor(' ', x + i, y, &black, &black);
		}
		printString(defaultEntry, x, y, &whiteBrogue, &black, 0);
	} else {
		confirmMessages();
	...
	do {
		printString(suffix, charNum + x, y, &grayBrogue, &black, 0);
		plotCharWithColor((suffix[0] ? suffix[0] : ' '), x + charNum, y, &black, &whiteBrogue);
		keystroke = nextKeyPress(true);
		if (keystroke == DELETE_KEY && charNum > 0) {
			printString(suffix, charNum + x - 1, y, &grayBrogue, &black, 0);
			...
			plotCharWithColor(keystroke, x + charNum, y, &whiteBrogue, &black);
			printString(suffix, charNum + x + 1, y, &grayBrogue, &black, 0);
...
boolean confirm(char *prompt, boolean alsoDuringPlayback) {
	...
	encodeMessageColor(whiteColorEscape, 0, &whiteBrogue);
	encodeMessageColor(yellowColorEscape, 0, KEYBOARD_LABELS ? &yellow : &whiteBrogue);
	...
	retVal = printTextBox(prompt, COLS/3, ROWS/3, COLS/3, &whiteBrogue, &interfaceBoxColor, rbuf, buttons, 2);
	overlayDisplayBuffer(rbuf, NULL);
...
void refreshSideBar(short focusX, short focusY, boolean focusedEntityMustGoFirst) {
	...
	if (rogue.playbackMode) {
		printString("   -- PLAYBACK --   ", 0, printY++, &whiteBrogue, &black, 0);
		...
		if (rogue.playbackOOS) {
			...
			printString("      [PAUSED]      ", 0, printY++, &grayBrogue, &black, 0);
		}
		printString("                    ", 0, printY++, &whiteBrogue, &black, 0);
	}
	...
	if (gotFocusedEntityOnScreen) {
		...
		for (i=printY; i< ROWS - 1; i++) {
			printString("                    ", 0, i, &whiteBrogue, &black, 0);
		}
		sprintf(buf, "  -- Depth: %i --%s   ", rogue.depthLevel, (rogue.depthLevel < 10 ? " " : ""));
		printString(buf, 0, ROWS - 1, &whiteBrogue, &black, 0);
...
void printHelpScreen() {
	...
	for (i=0; i<BROGUE_HELP_LINE_COUNT; i++) {
		for (j=0; helpText[i][j]; j++) {
			if (helpText[i][j] == '*') {
				j = encodeMessageColor(helpText[i], j, &whiteBrogue);
			}
		}
	}
...
void printHighScores(boolean hiliteMostRecent) {
	...
	for (i = 0; i < HIGH_SCORES_COUNT && list[i].score > 0; i++) {
		...
		if (i == hiliteLineNum) {
			...
			applyColorAverage(&scoreColor, &whiteBrogue, 100);
...
void printProgressBar(short x, short y, const char barLabel[COLS], long amtFilled, long amtMax, color *fillColor, boolean dim) {
	...
	for (i=0; i<20; i++) {
		...
		textColor = (dim ? grayBrogue : whiteBrogue);
...
short printMonsterInfo(creature *monst, short y, boolean dim, boolean highlight) {
	...
	if (y < ROWS - 1) {
		printString("                    ", 0, y, &whiteBrogue, &black, 0); // Start with a blank line
		...
		printString("                   ", 1, y, &whiteBrogue, &black, 0);
		printString(buf, 1, y++, (dim ? &grayBrogue : &whiteBrogue), &black, 0);
	}
    ...
    if (y < ROWS - 1
        ...
        buf[24] = '\0';
		printString(buf, 0, y++, (dim ? &grayBrogue : &whiteBrogue), &black, 0);
    }
		...
		&& (!(monst->info.flags & MONST_INANIMATE)
			...
			if (y < ROWS - 1) {
				if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience && y < ROWS - 1) {
					printString(hallucinationStrings[rand_range(0, 9)], 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
				} else if (monst->bookkeepingFlags & MB_CAPTIVE && y < ROWS - 1) {
					printString("     (Captive)      ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
				} else if ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID)
						   && !cellHasTMFlag(monst->xLoc, monst->yLoc, TM_ALLOWS_SUBMERGING)) {
					printString("     (Helpless)     ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
				} else if (monst->creatureState == MONSTER_SLEEPING && y < ROWS - 1) {
					printString("     (Sleeping)     ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
                } else if ((monst->creatureState == MONSTER_ALLY) && y < ROWS - 1) {
                    printString("       (Ally)       ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
				} else if (monst->creatureState == MONSTER_FLEEING && y < ROWS - 1) {
					printString("     (Fleeing)      ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
					...
					if ((monst->bookkeepingFlags & MB_FOLLOWER) && monst->leader && (monst->leader->info.flags & MONST_IMMOBILE)) {
						...
						printString("    (Worshiping)    ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
						...
						printString("     (Guarding)     ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
					} else {
						printString("    (Wandering)     ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
					}
                } else if (monst->ticksUntilTurn > max(0, player.ticksUntilTurn) + player.movementSpeed) {
                    printString("   (Off balance)    ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
                } else if ((monst->creatureState == MONSTER_TRACKING_SCENT) && y < ROWS - 1) {
					printString("     (Hunting)      ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
				}
			}
			...
			if (y < ROWS - 1) {
				...
				if (player.status[STATUS_WEAKENED]) {
					...
					encodeMessageColor(grayColorEscape, 0, (dim ? &darkGrayBrogue : &grayBrogue));
				}
				...
				//buf[20] = '\0';
				printString("                    ", 0, y, &whiteBrogue, &black, 0);
				printString(buf, (20 - strLenWithoutEscapes(buf)) / 2, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
			}
            ...
            if (y < ROWS - 1) {
                ...
                sprintf(buf, "%sStealth range: %i%s",
                        ...
                        grayColorEscape);
				printString("                    ", 0, y, &whiteBrogue, &black, 0);
				printString(buf, 1, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
            }
		}

	if (y < ROWS - 1) {
		printString("                    ", 0, y++, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
	}
	...
	if (highlight) {
		for (i=0; i<20; i++) {
			...
			for (j=initialY; j < (y == ROWS - 1 ? y : min(y - 1, ROWS - 1)); j++) {
				highlightScreenCell(i, j, &whiteBrogue, highlightStrength);
			}
		}
	}
...
short printItemInfo(item *theItem, short y, boolean dim, boolean highlight) {
	...
	if (y < ROWS - 1) {
		...
		printString(":                  ", 1, y, (dim ? &grayBrogue : &whiteBrogue), &black, 0);
		if (rogue.playbackOmniscience || !player.status[STATUS_HALLUCINATING]) {
			itemName(theItem, name, true, true, (dim ? &grayBrogue : &whiteBrogue));
		} else {
		...
		for (i=initialY + 1; i <= initialY + lineCount + 1 && i < ROWS - 1; i++) {
			printString("                    ", 0, i, (dim ? &darkGrayBrogue : &grayBrogue), &black, 0);
		}
		y = printStringWithWrapping(name, 3, y, 20-3, (dim ? &grayBrogue : &whiteBrogue), &black, NULL); // Advances y.
	}
...
short printTerrainInfo(short x, short y, short py, const char *description, boolean dim, boolean highlight) {
	...
	if (py < ROWS - 1) {
		...
		printString(":                  ", 1, py, (dim ? &grayBrogue : &whiteBrogue), &black, 0);
...
void printMonsterDetails(creature *monst, cellDisplayBuffer rbuf[COLS][ROWS]) {
	...
	printTextBox(textBuf, monst->xLoc, 0, 0, &whiteBrogue, &black, rbuf, NULL, 0);
}
...
unsigned long printCarriedItemDetails(item *theItem,
	...
	b = 0;
	if (includeButtons) {
		encodeMessageColor(goldColorEscape, 0, KEYBOARD_LABELS ? &yellow : &whiteBrogue);
		encodeMessageColor(whiteColorEscape, 0, &whiteBrogue);
	...
	b = printTextBox(textBuf, x, y, width, &whiteBrogue, &interfaceBoxColor, rbuf, buttons, b);
...
void printFloorItemDetails(item *theItem, cellDisplayBuffer rbuf[COLS][ROWS]) {
	...
	itemDetails(textBuf, theItem);
	printTextBox(textBuf, theItem->xLoc, 0, 0, &whiteBrogue, &black, rbuf, NULL, 0);
}
