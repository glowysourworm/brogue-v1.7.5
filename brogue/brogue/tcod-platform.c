...
//		/* ------------------------------------------------------------------
//
//		MIGRATION TO SDL 2 (3 forthcoming for multiple windows)
//
//		------------------------------------------------------------------ */
//
...
//
//        // SDL 2:  Texture method (tries for hardware acceleration)
//
...
//
//        // Update the texture from the pixel array
//        SDL_UpdateTexture(sdlTexture, NULL, myPixels, 640 * sizeof (Uint32));
//
//        // Apply the texture using the renderer
//        SDL_RenderClear(sdlRenderer);
//        SDL_RenderCopy(sdlRenderer, sdlTexture, NULL, NULL);
//        SDL_RenderPresent(sdlRenderer);
//
//
//        // SDL 2:  Blit method (renders your pixel arrays to one or many textures)
//
//        /*
//            This scenario has your SDL 1.2 game loading a bunch of graphics from
//            disk into a bunch of SDL_Surfaces, possibly trying to get them into
//            video RAM with SDL_HWSURFACE. You load these once, and you blit them
//            over and over to the framebuffer as necessary, but otherwise they never
...
//
//        /*
//            Use this, and you load your SDL_Surface as usual, but then at the end
//            you make a texture out of it. Once you have an SDL_Texture, you can
//            free the original surface.
//
//            At this point, your 1.2 game had a bunch of SDL_Surfaces, which it would
//            SDL_BlitSurface() to the screen surface to compose the final framebuffer,
//            and eventually SDL_Flip() to the screen. For SDL 2.0, you have a bunch of
//            SDL_Textures, that you will SDL_RenderCopy() to your Renderer to compose
//            the final framebuffer, and eventually SDL_RenderPresent() to the screen.
//            It's that simple. If these textures never need modification, you might find
...
//
		...
		if (windowCount != 1)
            throw("Mishandled SDL Window API call:  invalid window count");

        // Call SDL API for main window size
		SDL_GetWindowSize(windows[0], windowWidth, windowHeight);

        // Get integer data sent back from SDL from the heap
        int screenWidth = *windowWidth;
        int screenHeight = *windowHeight;


        //const SDL_VideoInfo* vInfo =
		//int screenWidth = desktop_width = get_sdl_window()->w;
		//int screenHeight = desktop_height = get_sdl_window()->h;
...
static void tcod_nextKeyOrMouseEvent(rogueEvent *returnEvent, boolean textInput, boolean colorsDance)
{
	...
	for (;;) {
		...
		if (!(serverMode || (SDL_GetAppState() & SDL_APP_WILLENTERFOREGROUND))) {
