...
static Lab toLab(CIE *c) {
	CIE n = (CIE) {Labf(c->X / whiteBrogue.X), Labf(c->Y / whiteBrogue.Y), Labf(c->Z / whiteBrogue.Z)};
	Lab l;
...
static CIE adams(CIE *v) {
	...
	c.X = munsellSloanGodlove((whiteBrogue.Y / whiteBrogue.X) * v->X) - c.Y;
	c.Z = munsellSloanGodlove((whiteBrogue.Z / whiteBrogue.X) * v->Z) - c.Y;
