...
void welcome() {
    ...
    encodeMessageColor(buf, strlen(buf), &whiteBrogue);
...
void generateFontFiles() {
	...
	for (i=0; i<COLS; i++) {
		for(j=0; j<ROWS; j++ ) {
			plotCharWithColor(' ', i, j, &whiteBrogue, &whiteBrogue);
		}
	}
	i = j = 0;
	for (k=0; k<256; k++) {
		...
		if (j == 8) {
			plotCharWithColor(c8[i], i, j+5, &whiteBrogue, &black);
		} else if (j == 9) {
			plotCharWithColor(c9[i], i, j+5, &whiteBrogue, &black);
		} else {
			plotCharWithColor(k, i, j+5, &whiteBrogue, &black);
		}
	}
	for (;;) {
...
void gameOver(char *killedBy, boolean useCustomPhrasing) {
	...
	if (rogue.quit) {
        ...
        strcpy(buf, "You die...");
        if (KEYBOARD_LABELS) {
            encodeMessageColor(buf, strlen(buf), &veryDarkGrayBrogue);
	...
	if (!rogue.quit) {
        printString(buf, (COLS - strLenWithoutEscapes(buf)) / 2, ROWS / 2, &grayBrogue, &black, 0);
...
void victory(boolean superVictory) {
    ...
    if (superVictory) {
        ...
        displayMoreSign();
        printString("Congratulations; you have transcended the Dungeons of Doom!                 ", mapToWindowX(0), mapToWindowY(-1), &black, &whiteBrogue, 0);
        displayMoreSign();
        ...
        copyDisplayBuffer(dbuf, displayBuffer);
        funkyFade(dbuf, &whiteBrogue, 0, 240, mapToWindowX(player.xLoc), mapToWindowY(player.yLoc), false);
        displayMoreSign();
        printString("Congratulations; you have escaped from the Dungeons of Doom!     ", mapToWindowX(0), mapToWindowY(-1), &black, &whiteBrogue, 0);
        displayMoreSign();
        clearDisplayBuffer(dbuf);
        deleteMessages();
	...
	printString(displayedMessage[0], mapToWindowX(0), mapToWindowY(-1), &whiteBrogue, &black, dbuf);

	printString("Gold", mapToWindowX(2), mapToWindowY(1), &whiteBrogue, &black, dbuf);
	...
	for (i = 4, theItem = packItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
        ...
        if (theItem->category == AMULET && superVictory) {
            ...
            identify(theItem);
            itemName(theItem, buf, true, true, &whiteBrogue);
            upperCase(buf);
            printString(buf, mapToWindowX(2), min(ROWS-1, i + 1), &whiteBrogue, &black, dbuf);
	...
	funkyFade(dbuf, &whiteBrogue, 0, 120, COLS/2, ROWS/2, true);
