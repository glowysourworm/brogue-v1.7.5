...
void drawMenuFlames(signed short flames[COLS][(ROWS + MENU_FLAME_ROW_PADDING)][3], unsigned char mask[COLS][ROWS]) {
	...
	for (j=0; j<ROWS; j++) {
		for (i=0; i<COLS; i++) {
			...
			if (mask[i][j] == 100) {
				plotCharWithColor(dchar, i, j, &darkGrayBrogue, maskColor);
			} else {
				tempColor = black;
				...
				plotCharWithColor(dchar, i, j, &darkGrayBrogue, &tempColor);
			}
		}
	}
}
...
void dialogAlert(char *message) {
	...
	printTextBox(message, COLS/3, ROWS/3, COLS/3, &whiteBrogue, &interfaceBoxColor, rbuf, &OKButton, 1);
